(function() {
	...
	hm.newJqEvent( "review", "click", function( e ) {
	...
	hm.App.add( {
		...
		loadModel: function( hmModelContainer, options ) {
			...
			var modelNode = this.getModelNode( hmModelContainer );

			var data = modelNode.getLocal( "entries" );

			if (data) {
				var localVersion = modelNode.getLocal( "version" );
				...
				if (localVersion != version) {
					...
					if (ok) {
						...
						return $.getJSON( "doc-json.js" ).done( function( data ) {
							buildModel( data, modelNode );
							entriesNode.saveLocal();
						} );

					} else {
						return buildModel( data, modelNode );
					}
				} else {
					return buildModel( data, modelNode );
				}
	...
	function buildModel( entries, appNode ) {
		...
		appNode.set( {
			...
			handlers: {
				...
				expandParentNodeForSelectedEntry: function( e ) {
					...
					this.find( "a[tab-link]" ).each( function() {
						var $this = $( this );
						if (selectedEntryName.startsWith( $this.attr( "tab-link" ) )) {
		...
		hm.routeParams( apiActionNode.path, selectedEntryNameNode.path );
